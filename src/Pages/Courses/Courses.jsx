      ...
      <div className="max-w-7xl mx-auto px-3">
        ...
        </div>
        {
          ...
          {courses.map((course) => (
            ...
            <figure>
              <img className="cardImage" src={course.image} alt="Course Image" />
            </figure>
            <div className="badgeContainer">
              <p className={`px-3 absolute -mt-6 text-white py-1 text-sm rounded font-semibold ${course.badge_color}`}>{course.badge_text}</p>
            </div>
            <div className="myCardBody">
              <h2 className="myCardTitle">{course.title}</h2>
              <p className="description">{course.description.slice(0, 45)}...</p>
        ...
        :
        <h3 className="text-center py-16 dark2 text-3xl font-bold">No Item to Show.....</h3>
        }
      </div>
    </div>
  );
